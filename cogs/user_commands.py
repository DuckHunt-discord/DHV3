    ...
    async def bang(self, ctx, target: discord.Member = None):
        ...
        for duck in self.bot.ducks_spawned:
            if duck.channel == channel and not duck.killed:
                ...
                current_duck = duck
                diag_slowness(now, ctx, "Found duck")
                break

        else:
            diag_slowness(now, ctx, "No duck")
            ...
            if await self.bot.db.get_stat(message.channel, message.author,
                ...
                await self.sendBangMessage(ctx, _(
                    ...
                    language))
                diag_slowness(now, ctx, "Sent no duck message")
        ...
        diag_slowness(now, ctx, "Wrapped duck")
        ...
        if duck_will_leave:
            ...
            return
        diag_slowness(now, ctx, "Duck didn't leave")
        ...
        if (chance > accuracy and duck.can_miss) or target:
            diag_slowness(now, ctx, "Duck missed")
