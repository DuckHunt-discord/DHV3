...
class SuperAdmin:
    ...
    async def db_id(self, ctx):
        ...
        await self.bot.send_message(ctx=ctx,
                                    message=f"Channel ID in the DB is {dbid}")

    @commands.command()
    @checks.is_super_admin()
    async def get_level_from_exp(self, ctx, exp:int):
        await self.bot.send_message(ctx=ctx,
                                    message=await self.bot.db.get_level(exp))
    ...
    async def broadcast(self, ctx, *, bc: str):
        """!broadcast [message]"""
        await self.bot.send_message(ctx=ctx,
                                    message="Starting the broadcast...")
        ...
        await self.bot.send_message(ctx=ctx,
                                    message="Broadcast finished!", return_message=True) # Rate limits
